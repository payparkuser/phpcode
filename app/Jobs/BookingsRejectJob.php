...
class BookingsRejectJob implements ShouldQueue
{
   ...
   /**
    ...
    public function handle()
    {
        try {
            ...
            $title = $content = Helper::push_message(615);

            $data['space_id'] = $booking_details->space_id;

            $data['notification_type'] = BELL_NOTIFICATION_TYPE_BOOKINGS_REJECTED;
            ...
            if (Setting::get('is_email_notification') == YES && $user_details) {
                ...
                $email_data['subject'] = Setting::get('site_name').'-'.tr('provider_reject_booking_subject', $booking_details->unique_id);

                $email_data['page'] = "emails.providers.bookings.approve";
                ...
                $data['space_details'] = $booking_details->hostDetails->toArray() ?? [];

                $data['status'] = tr('rejected');
