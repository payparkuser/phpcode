...
class BookingsCreateJob implements ShouldQueue
{
   ...
   /**
    ...
    public function handle()
    {
        try {
            ...
            $title = $content = Helper::push_message(616);

            $data['space_id'] = $booking_details->host_id;

            $data['notification_type'] = BELL_NOTIFICATION_TYPE_BOOKING_DONE_BY_USER;
            ...
            if(Setting::get('is_email_notification') == YES && $provider_details) {
                ...
                $email_data['subject'] = Setting::get('site_name').'-'.tr('bookings_created_for_the_space');
