...
class BookingsApproveJob implements ShouldQueue
{
   ...
   /**
    ...
    public function handle()
    {
        try {
            ...
            $title = $content = Helper::push_message(614);

            $data['space_id'] = $booking_details->space_id;

            $data['notification_type'] = BELL_NOTIFICATION_TYPE_BOOKING_APPROVED;
            ...
            if (Setting::get('is_email_notification') == YES && $user_details) {
                ...
                $email_data['subject'] = Setting::get('site_name').'-'.tr('provider_approve_booking_subject', $booking_details->unique_id);
