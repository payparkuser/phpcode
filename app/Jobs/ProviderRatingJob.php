...
class ProviderRatingJob implements ShouldQueue
{
    ...
    /**
    * The number of times job myy attempted
    *
    * @var int
    */
    public $tries = 2;
    ...
    public function handle()
    {
         try {
            ...
            $data['notification_type'] = BELL_NOTIFICATION_TYPE_PROVIDER_REVIEW;
            ...
            if (Setting::get('is_push_notification') == YES && $user_details) {
                ...
                if($user_details->push_notification_status == YES && ($user_details->device_token != '')) {
                    ...
                    \Notification::send($booking_details->id, new \App\Notifications\PushNotification($title , $content, $push_data, $user_details->device_token));
                }
            }
